...
public class OrderController : Controller
{
        ...
        string? dateRange = null)
    {
        ...
        using (var package = new ExcelPackage())
        {
            ...
            // Add logo image
            string logoPath = Path.Combine(_webHostEnvironment.WebRootPath, "images", "pizzashop_logo.png");
            ...
            // Status Value
            worksheet.Cells[2, 2].Value = status.HasValue ? ((OrderStatus)status).ToString() : "All";
            ...
            {
                dateRangeValue = ((DateRange)drangeValue).ToString();
            }
            ...
            if (orders.orders != null)
            {
                foreach (var order in orders.orders)
                {
                    ...
                    worksheet.Cells[row, 2].Value = order.Createdat?.ToString("yyyy-MM-dd HH:mm:ss");
